// implementaion of heap and max heap
#include<stdio.h>
#include<stdlib.h>
...
void minHeapify1(struct node* root);
...
struct node * minheapinsert(struct node* root,int data){
...
int  mindelete(struct node*root){
    if(root==NULL){
        return 0;
    }
    else{
...
int main(){
    ...
    for (int i = 0; i < n; i++) {
        root = minheapinsert(root, keys[i]);
    }
